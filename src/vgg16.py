...
class VGG16(object):
    ...
    def conv_layer(self, bottom, name, trainable=False):
        with tf.compat.v1.variable_scope(name):
            ...
            conv_weights = tf.compat.v1.get_variable("W", shape=w.shape, initializer=tf.constant_initializer(w),
                                                     trainable=trainable)
            conv_biases = tf.compat.v1.get_variable("b", shape=b.shape, initializer=tf.constant_initializer(b),
                                                    trainable=trainable)
